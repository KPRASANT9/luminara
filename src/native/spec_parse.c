    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strncmp(line, "substrate ", 10) == 0 && strchr(line, '{')) {
            if (spec->substrate_count < CSOS_MAX_SUBSTRATES) {
                ...
                /* Defaults: rw derived from atom DOF at runtime, calvin from CO2 pressure.
                 * These hints are parsed but NOT used — all runtime values come from
                 * csos_derive_*() functions. Retained for spec compatibility only. */
                cur_sub->rw_hint = 0.0;       /* 0 = "derive at runtime" */
                cur_sub->calvin_freq_hint = 0; /* 0 = "derive at runtime" */
                in_substrate = 1;
            }
            continue;
        }
...
csos_membrane_t *csos_membrane_from_spec(const csos_spec_t *spec, int ring_index) {
    ...
    m->atom_count = count;
    /* Dynamic RW from Gouterman: start at population floor (derived from atom DOF) */
