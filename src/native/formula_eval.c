    ...
    if (p->cur.type == TOK_IDENT) {
        ...
        if (p->cur.type == TOK_LPAREN) {
            ...
            if (strcmp(name, "abs") == 0) result = fabs(arg1);
                ...
                /* IEEE 754 double: exp(709.8) ≈ DBL_MAX, exp(710) = inf.
                 * Clamp at 700 — the machine arithmetic limit, not a physics constant. */
                double clamp = 700.0;
                double clamped = arg1 > clamp ? clamp : (arg1 < -clamp ? -clamp : arg1);
                result = exp(clamped);
            }
