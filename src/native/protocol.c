...
int csos_handle(csos_organism_t *org, const char *json_in,
    ...
    /* ═══ DELIVER: Auto-egress based on decision type + egress.json ═══ */
    if (strcmp(action, "deliver") == 0) {
        ...
        /* Read egress.json for routing */
        char epath[512];
        snprintf(epath, sizeof(epath), "%s/.csos/egress.json", org->root);
        ...
        /* Absorb the delivery event */
        csos_photon_t ph = csos_organism_absorb(org, "deliverable", content, PROTO_INTERNAL);
    ...
    if (strcmp(action, "tool") == 0) {
        char filepath[512] = {0}, filecontent[16384] = {0};
